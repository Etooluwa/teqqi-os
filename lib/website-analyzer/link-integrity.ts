...
import type {
  ...
  InvalidLinkEvidence,
  LinkFact,
  LinkIntegrityEvidence,
  LinkProbeEvidence,
  PageFacts,
  UnsupportedProtocolEvidence,
...
function isSameSite(url: string, siteUrl: string): boolean {
  return hostnameBase(new URL(url).hostname) === hostnameBase(new URL(siteUrl).hostname);
}

function classifyLinks(pageUrl: string, links: LinkFact[], siteUrl: string, output: {
  internal: Set<string>;
  external: Set<string>;
  invalid: InvalidLinkEvidence[];
  unsupported: UnsupportedProtocolEvidence[];
}) {
  for (const link of links) {
    const href = link.href;
    if (href === null) {
      output.invalid.push({ pageUrl, href: null, reason: "missing" });
      continue;
    }
    if (!href.trim() || href.trim() === "#") {
      output.invalid.push({ pageUrl, href, reason: "empty" });
      continue;
    }
    ...
    try {
      ...
      output.invalid.push({ pageUrl, href, reason: "malformed" });
      continue;
    }

    if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
      if (!ALLOWED_NON_HTTP_PROTOCOLS.has(parsed.protocol)) {
        output.unsupported.push({ pageUrl, href, protocol: parsed.protocol || "unknown" });
      }
      continue;
    }
    ...
    if (isSameSite(normalized, siteUrl)) output.internal.add(normalized);
    else output.external.add(normalized);
  }
}

async function probeExternal(url: string): Promise<LinkProbeEvidence> {
  try {
    const target = await validateWebsiteUrl(url);
    ...
    return {
      ...
      statusCode: null,
      finalUrl: null,
      error: timeout ? "External link probe timed out." : error instanceof Error ? error.message : "External link probe failed.",
    };
  }
}

export async function collectLinkIntegrityEvidence(input: {
  finalUrl: string;
  homepageFacts: PageFacts | null;
  crawlability: CrawlabilityEvidence;
}): Promise<LinkIntegrityEvidence> {
  const internal = new Set<string>();
  const external = new Set<string>();
  const invalid: InvalidLinkEvidence[] = [];
  const unsupported: UnsupportedProtocolEvidence[] = [];
  let totalLinks = 0;
  let sourcePageCount = 0;

  if (input.homepageFacts) {
    sourcePageCount += 1;
    totalLinks += input.homepageFacts.links.length;
    classifyLinks(input.finalUrl, input.homepageFacts.links, input.finalUrl, { internal, external, invalid, unsupported });
  }
  ...
  for (const page of input.crawlability.internalCrawl.pages) {
    ...
    classifyLinks(page.finalUrl, page.links, input.finalUrl, { internal, external, invalid, unsupported });
  }
  ...
  return {
    ...
    externalProbeLimit: EXTERNAL_PROBE_LIMIT,
    externalProbeTruncated: externalUrls.length > EXTERNAL_PROBE_LIMIT,
  };
}
