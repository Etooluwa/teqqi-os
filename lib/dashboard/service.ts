...
function bestOpportunity(result: WebsiteOpportunityEngineResult): DashboardBestOpportunity {
  ...
  return opportunity ? { opportunityId: opportunity.opportunityId, title: opportunity.title, priority: opportunity.priority, confidence: opportunity.confidence, recommendedService: opportunity.recommendedService } : null;
}
...
async function loadScoringRuns(): Promise<Map<string, number | null>> {
  const rows = await supabaseRest<ScoringRunRow[]>("/website_scoring_runs?status=eq.COMPLETED&select=id,website_score&order=created_at.desc&limit=1000");
...
export async function buildOpportunityDashboardSnapshot(
  ...
  const [references, latestOpportunityRuns, scoringRuns] = await Promise.all([getSearchPlaceReferences(selectedSearch.id), loadLatestOpportunityRuns(), loadScoringRuns()]);
  const resultByOpportunityRunId = new Map([...latestOpportunityRuns.values()].map((run) => [run.id, run.result] as const));

  const businesses: DashboardBusinessRow[] = await Promise.all(references.map(async (reference) => {
    try {
