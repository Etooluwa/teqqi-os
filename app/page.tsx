      ...
      <header className="sticky top-0 z-20 border-b border-slate-200/90 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div><p className="font-semibold leading-none">TEQQI OS</p><p className="mt-1 text-xs text-slate-500">Opportunity intelligence dashboard</p></div>
          </div>
      ...
      <div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        {showDiscovery && (
          <section className="mb-6 rounded-2xl border border-indigo-200 bg-white p-5 shadow-sm">
            <div className="mb-4"><p className="font-semibold">Discover a new market</p><p className="mt-1 text-sm text-slate-500">New searches return previously unseen Google Place IDs.</p></div>
            <form onSubmit={runDiscovery} className="grid gap-3 md:grid-cols-[1fr_1fr_140px_auto]">
              ...
              <select value={maxResults} onChange={(e) => setMaxResults(Number(e.target.value))} className="h-11 rounded-xl border border-slate-300 px-3 text-sm">{[5, 10, 20, 40, 60].map((value) => <option key={value} value={value}>{value} results</option>)}</select>
        ...
        {loading && !dashboard ? (
          <section className="rounded-2xl border border-slate-200 bg-white p-16 text-center shadow-sm"><div className="mx-auto mb-4 h-9 w-9 animate-spin rounded-full border-4 border-slate-200 border-t-slate-950" /><p className="font-semibold">Preparing opportunity intelligence…</p><p className="mt-1 text-sm text-slate-500">Refreshing live business details and connecting website intelligence.</p></section>
        ) : dashboard ? (
          <>
            <section className="mb-6 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
              <div><div className="mb-2 flex flex-wrap items-center gap-2"><span className="rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-semibold text-indigo-700">Phase 9 · Opportunity Dashboard</span><span className="text-xs text-slate-400">Dashboard v{dashboard.dashboardVersion}</span></div><h1 className="text-3xl font-bold tracking-tight sm:text-4xl">{dashboard.market.industry} in {dashboard.market.location}</h1><p className="mt-2 text-sm text-slate-500">Discovered {formatDate(dashboard.market.createdAt)} · Business details refreshed live from Google</p></div>
              <select value={dashboard.market.id} onChange={(e) => void switchMarket(e.target.value)} className="h-11 max-w-full rounded-xl border border-slate-300 bg-white px-3 text-sm font-medium shadow-sm lg:min-w-[320px]">{dashboard.historyNavigation.map((entry) => <option key={entry.id} value={entry.id}>{entry.industry} · {entry.location} · {formatDate(entry.createdAt)}</option>)}</select>
            </section>
            ...
            <section className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
              ...
              <SummaryCard label="Analyzed" value={dashboard.summary.businessesAnalyzed} note={`${dashboard.summary.analysisCoveragePercent}% market coverage`} />
              <SummaryCard label="Avg website score" value={dashboard.summary.averageWebsiteScore === null ? "—" : Math.round(dashboard.summary.averageWebsiteScore)} note={dashboard.summary.lowestWebsiteScore === null ? "No scored sites yet" : `Range ${Math.round(dashboard.summary.lowestWebsiteScore)}–${Math.round(dashboard.summary.highestWebsiteScore ?? 0)}`} />
              <SummaryCard label="Opportunities" value={dashboard.summary.totalOpportunities} note={`${dashboard.summary.businessesWithOpportunities} businesses affected`} />
            ...
            <section className="mb-6 grid gap-4 xl:grid-cols-[1.3fr_.7fr]">
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="font-semibold">Opportunity mix</h2><p className="mt-1 text-sm text-slate-500">Recommended website services across this market.</p></div><span className="text-xs font-medium text-slate-400">{dashboard.summary.opportunityCoveragePercent}% coverage</span></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{dashboard.summary.opportunityCountsByService.map((item) => <div key={item.service} className="rounded-xl bg-slate-50 px-4 py-3"><p className="text-sm font-medium text-slate-700">{serviceLabel(item.service)}</p><p className="mt-1 text-2xl font-bold">{item.count}</p></div>)}</div></div>
              <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-semibold">Website Score distribution</h2><p className="mt-1 text-sm text-slate-500">Analyzed businesses by score band.</p><div className="mt-5 space-y-3">{dashboard.summary.websiteScoreDistribution.map((band) => <div key={band.band} className="flex items-center justify-between rounded-xl bg-slate-50 px-4 py-3"><span className="text-sm font-medium text-slate-600">{band.label}</span><span className="font-bold">{band.count}</span></div>)}</div></div>
            </section>

            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="border-b border-slate-200 p-5"><div className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end"><div><h2 className="text-lg font-semibold">Ranked businesses</h2><p className="mt-1 text-sm text-slate-500">Prioritized using website opportunity evidence — not a commercial Lead Score.</p></div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:flex">
                <select value={priority} onChange={(e) => setPriority(e.target.value)} className="h-10 rounded-lg border border-slate-300 px-2.5 text-sm"><option value="">All priorities</option>{["CRITICAL", "HIGH", "MEDIUM", "LOW"].map((value) => <option key={value}>{value}</option>)}</select>
                <select value={confidence} onChange={(e) => setConfidence(e.target.value)} className="h-10 rounded-lg border border-slate-300 px-2.5 text-sm"><option value="">All confidence</option>{["HIGH", "MEDIUM", "LOW"].map((value) => <option key={value}>{value}</option>)}</select>
                <select value={service} onChange={(e) => void applyService(e.target.value)} className="h-10 rounded-lg border border-slate-300 px-2.5 text-sm"><option value="">All services</option>{serviceOptions.map((value) => <option key={value} value={value}>{serviceLabel(value)}</option>)}</select>
                <select value={analysis} onChange={(e) => setAnalysis(e.target.value)} className="h-10 rounded-lg border border-slate-300 px-2.5 text-sm"><option value="ALL">All analysis</option><option value="ANALYZED">Analyzed</option><option value="NOT_ANALYZED">Not analyzed</option><option value="HAS_WEBSITE">Has website</option><option value="NO_WEBSITE">No website</option></select>
              ...
              </div></div><div className="mt-4 flex flex-wrap items-center gap-2"><input value={minScore} onChange={(e) => setMinScore(e.target.value)} inputMode="numeric" placeholder="Min score" className="h-9 w-24 rounded-lg border border-slate-300 px-2.5 text-sm" /><input value={maxScore} onChange={(e) => setMaxScore(e.target.value)} inputMode="numeric" placeholder="Max score" className="h-9 w-24 rounded-lg border border-slate-300 px-2.5 text-sm" /><span className="ml-auto text-xs text-slate-400">Showing {dashboard.tableView.filteredRows} of {dashboard.tableView.totalRows}</span></div></div>

              <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Rank</th><th className="px-5 py-3">Business</th><th className="px-5 py-3">Website Score</th><th className="px-5 py-3">Best opportunity</th><th className="px-5 py-3">Priority</th><th className="px-5 py-3">Confidence</th><th className="px-5 py-3">Opportunities</th><th className="px-5 py-3">Status</th></tr></thead><tbody className="divide-y divide-slate-100">
                {rows.map((row) => <tr key={row.externalId} className="align-top transition hover:bg-slate-50/70"><td className="px-5 py-4 text-sm font-bold text-slate-400">#{row.rank}</td><td className="max-w-[300px] px-5 py-4"><p className="font-semibold">{row.businessName}</p><p className="mt-1 truncate text-xs text-slate-500">{row.formattedAddress ?? "Address unavailable"}</p>{row.websiteUrl && <a href={row.websiteUrl} target="_blank" rel="noreferrer" className="mt-2 inline-block text-xs font-semibold text-indigo-600 hover:underline">Visit website ↗</a>}</td><td className="px-5 py-4"><Score value={row.websiteScore} /></td><td className="max-w-[280px] px-5 py-4"><p className="text-sm font-medium">{row.bestOpportunity?.title ?? "No eligible opportunity"}</p><p className="mt-1 text-xs text-slate-400">{row.bestOpportunity ? serviceLabel(row.bestOpportunity.recommendedService) : row.intelligenceAvailable ? "Analyzed" : "Awaiting analysis"}</p></td><td className="px-5 py-4">{row.bestOpportunity ? <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${priorityClasses[row.bestOpportunity.priority]}`}>{row.bestOpportunity.priority}</span> : <span className="text-sm text-slate-400">—</span>}</td><td className="px-5 py-4 text-sm font-semibold text-slate-600">{row.bestOpportunity?.confidence ?? "—"}</td><td className="px-5 py-4 text-sm font-bold">{row.opportunityCount}</td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${row.intelligenceAvailable ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{row.intelligenceAvailable ? "Analyzed" : "Not analyzed"}</span></td></tr>)}
              </tbody></table></div>
