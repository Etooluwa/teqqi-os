...
if (source === "NO_INTELLIGENCE_AVAILABLE") {
  console.log("↪ No live or persisted opportunity intelligence is available; validating the explicit unavailable state.");
}
...
if (source === "NO_INTELLIGENCE_AVAILABLE") {
  assert(recommendations.available === false, "Recommendations must remain explicitly unavailable when no opportunity run exists.");
  assert(typeof recommendations.unavailableReason === "string" && recommendations.unavailableReason.length > 0, "Unavailable recommendations must explain why they are unavailable.");
  assert(recommendations.opportunityCount === 0, "Unavailable recommendations must not fabricate opportunity counts.");
  assert(Array.isArray(recommendations.recommendations) && recommendations.recommendations.length === 0, "Unavailable recommendations must not fabricate recommendation cards.");
  assert(detail.intelligence.opportunityRun === null, "No opportunity run should be fabricated when recommendations are unavailable.");
  assert(detail.leadScore.available === false && detail.leadScore.score === null, "Missing recommendations must not create commercial Lead Scoring.");
  console.log("✓ Missing recommendation intelligence is explicit and does not fabricate opportunities, evidence, or Lead Score");
} else {
